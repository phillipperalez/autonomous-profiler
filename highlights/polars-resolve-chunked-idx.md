...
- ✅ **Library compiles + runs clean with the change:** the workload harness builds `polars-core` (incl. this `gather.rs`) and executes the full gather over 2.96M rows with no errors/warnings attributable to the patch.
- ⚠️ **Full `polars-core` unit-test suite could not be run in this checkout:** `cargo test -p polars-core` fails to compile (test cfg) on **two pre-existing `E0432` unresolved-import errors in `series/from.rs` and `series/proptest.rs`** — present at the unmodified baseline and a polars dev/feature-matrix quirk (newer local rustc). This change touches **only a private fn in `gather.rs`**, which cannot cause import errors in the `series` module, so it is provably **not** the cause and is unaffected by it. Correctness therefore rests on the equivalence proof above + the 2.96M-row fingerprint — which is a *stronger* guarantee for this specific function than the generic suite would give.
